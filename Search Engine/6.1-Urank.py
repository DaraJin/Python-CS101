...
def compute_ranks(graph):
	...
	npages=len(graph)
	for page in graph:
		ranks[page]=1.0/npages #initial rank score, when numloops=0,base case for recursion

	for i in range(0,numloops):
		newranks={}
		for page in graph:
            newrank = (1 - d) / npages #(1-d):score given by 0.2 randomness from outside
            for node in graph:
				if page in graph[node]: # if this_page appears in others(nodes)' url link---node links to page
					newrank = newrank + d * (ranks[node]/len(graph[node])) #ranks[node],last time ranks update(numloop)
...
#rank(page, t) = (1-d)/npages + sum (d * rank(p, t - 1) / number of outlinks from p) over all pages p that link to this page	
